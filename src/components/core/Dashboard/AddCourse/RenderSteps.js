...
//   console.log("step", step);
  ...
  return (
    <div>
                ...
                return (<>
                    <div key={item.id} className="flex flex-col items-center ">
                        ...
                        ${step > item.id && "bg-yellow-50 text-yellow-50"}}`}>
                            {
                                step > item.id ? (<FaCheck className="font-bold text-richblack-900"/>) : (<div>{item.id}</div>)
                            }
                        </div>
                    </div>    
        ...
        {
            steps.map((item) => {
                return (<div key={item.id} className="flex min-w-[130px] flex-col items-center gap-y-2">
                    <p className={`text-sm ${
